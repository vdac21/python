...
cursor.execute('''insert into DATA(policy ,expirydate,location,state,region,insurencevalue,construction,businesstype,eartquake,flood) values
              ...
              (100309,'2021-02-05','Urban','WI','Midwest',5000368,'Mansonry','Manufactur','N','N')
...
cursor.execute('''alter table DATA add unique (Insurencevalue)''')
cursor.execute('''alter table DATAadd id int identity(1,1)''')
#conn.commit()
